...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        [Obsolete("Use evtSteamInitalized instead")]
        public UnityEvent onInitalized => evtSteamInitalized;
        [FormerlySerializedAs("onInitalized")]
        public UnityEvent evtSteamInitalized;
        [Obsolete("Use evtSteamInitalizationError instead")]
        public UnityStringEvent onInitalizationError => evtSteamInitalizationError;
        [FormerlySerializedAs("onInitalizationError")]
        ...
        public void Init()
        {
            ...
            catch (System.DllNotFoundException e)
            {
                ...
                evtSteamInitalizationError.Invoke("[Steamworks.NET] Could not load [lib]steam_api.dll/so/dylib. It's likely not in the correct location. Refer to the README for more details.\n" + e);
                Application.Quit();
                return;
            }
            ...
            if (server.gameServerShuttingDown == null)
                server.gameServerShuttingDown = new UnityEvent();
            if (server.disconnected == null)
                server.disconnected = new GameServer.DisconnectedEvent();
            if (server.connected == null)
                server.connected = new GameServer.ConnectedEvent();
            if (server.failure == null)
                server.failure = new GameServer.FailureEvent();
            ...
            if (isDebugging)
                Debug.Log("Initalizing Steam Game Server API: (" + server.ip + ", " + server.gamePort.ToString() + ", " + server.queryPort.ToString() + ", " + eMode.ToString() + ", " + server.serverVersion + ")");
            ...
            SteamGameServer.SetModDir(server.gameDirectory);
            SteamGameServer.SetProduct(applicationId.m_AppId.ToString());
            SteamGameServer.SetGameDescription(server.gameDescription);
            SteamGameServer.SetMaxPlayerCount(server.maxPlayerCount);
            SteamGameServer.SetPasswordProtected(server.isPasswordProtected);
            SteamGameServer.SetServerName(server.serverName);
            SteamGameServer.SetBotPlayerCount(server.botPlayerCount);
            SteamGameServer.SetMapName(server.mapName);
            SteamGameServer.SetDedicatedServer(server.isDedicated);

            if (isDebugging)
            {
                Debug.Log("Configuring the SteamGameServer interface:\n\tServer Name: " + server.serverName +
                    "\n\tDescription: " + server.gameDescription +
                    "\n\tProduct: " + applicationId +
                    "\n\tIs Dedicated Server: " + server.isDedicated +
                    "\n\tIs Password Protected: " + server.isPasswordProtected +
                    "\n\tMax Players: " + server.maxPlayerCount +
                    "\n\tBot Player Count: " + server.botPlayerCount +
                    "\n\tMod Dir: " + server.gameDirectory +
                    "\n\tMap Name: " + server.mapName);
            }

            if (server.supportSpectators)
            {
                if (isDebugging)
                    Debug.Log("Spectator enabled:\n\tName = " + server.spectatorServerName + "\n\tSpectator Port = " + server.spectatorPort.ToString());

                SteamGameServer.SetSpectatorPort(server.spectatorPort);
                SteamGameServer.SetSpectatorServerName(server.spectatorServerName);
            }
            else if (isDebugging)
...
#pragma warning restore IDE0052 // Remove unread private members
            ...
            /// <summary>
            /// Access the Steam API's Matchmaking features including Steam Lobby, Lobby Chat, Lobby Query and more.
            /// </summary>
            /// <remarks>
            /// This is a simple wrapper around the <see cref="MatchmakingTools"/> object. It is slighhtly more performant to acceess the <see cref="MatchmakingTools"/> object directly.
            /// This object is here to help with developer discovery e.g. SteamSettings.Client.matchmaking and browsing for available features.
            /// </remarks>
            public MatchmakingTools.InstanceWrapper matchmaking;
            #endregion
#endif
