...
namespace HeathenEngineering.SteamAPI
{
    ...
    /// </summary>
    public static class SteamworksInventoryTools
    {
        ...
        private static CallResult<SteamInventoryRequestPricesResult_t> m_SteamInventoryRequestPricesResult;
        private static CallResult<SteamInventoryStartPurchaseResult_t> m_SteamInventoryStartPurchaseResult;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (SteamInventory.GetResultItems(param.m_handle, temp, ref count))
                    {
                        ...
                        {
                            Debug.LogError("Steamworks Inventory " + callerName + " failed to retrive the resulting Inventory Item details.");
                ...
                try
                {
                    if (param.m_result == EResult.k_EResultOK)
                    {
                        ...
                        if (SteamInventory.SerializeResult(param.m_handle, null, out count))
                        {
                            ...
                            {
                                Debug.LogError("Steamworks Inventory - Serialize Result: Failed to load the serialized results to memory.");
                                callRequest.SerializationCallback(false, null);
                            }
                        }
                        else
                        {
                            Debug.LogError("Steamworks Inventory - Serialize Result: Failed to calculate the size requirement for the serialized result.");
                            callRequest.SerializationCallback(false, null);
                        }
                    }
                    else
                    {
                        Debug.LogError("Steamworks Inventory - Serialize Result: Steamworks Result state: " + param.m_result.ToString() + ".");
                        callRequest.SerializationCallback(false, null);
                    }
                }
                catch (Exception ex)
                ...
                if (SteamInventory.GetResultItems(param.m_handle, temp, ref count))
                {
                    ...
                    if (responce)
                    {
                        try
                        {
                            ...
                            {
                                Debug.LogWarning("Deserialize results returned successfuly however found that the results did not match the Steamworks User ID and thus are invalid.\nThis is a security measure to insure users cannot spoof inventory results.");
                    ...
                    {
                        Debug.LogError("Steamworks Inventory Deserialize Result failed to retrive the resulting Inventory Item details.");
        ...
        private static void ProcessTransferRequest(SteamInventoryResultReady_t param, CallRequest callRequest)
        {
            try
            {
                ...
                else
                {
                    try
                    {
                        Debug.LogError("Steamworks Inventory - Transfer Item Quantity: Steamworks Result state: " + param.m_result.ToString() + ".");
                        if (callRequest.BoolCallback != null)
        ...
        private static void HandleSteamInventoryResult(SteamInventoryResultReady_t param)
        {
            ...
            {
                ProcessDetailQuery(param, new CallRequest { Type = CallRequestType.Unknown }, "Unsolicited Inventory Result");
            }
        }
        #endregion
        ...
        {
            var call = SteamInventory.RequestPrices();
            m_SteamInventoryRequestPricesResult.Set(call, callback.Invoke);
        }

        public static bool GetItemPrice(InventoryItemPointer pointer, out ulong currentPrice, out ulong basePrice)
        {
            return SteamInventory.GetItemPrice(pointer.definitionID, out currentPrice, out basePrice);
        }

        public static bool GetItemPrice(InventoryItemDefinition item, out ulong currentPrice, out ulong basePrice)
        {
            return SteamInventory.GetItemPrice(item.definitionID, out currentPrice, out basePrice);
        }
        ...
        /// <summary>
        /// Starts the purchase process for the user
        /// </summary>
        /// <remarks>
        /// <para>
        /// Starts the purchase process for the user, given a "shopping cart" of item definitions that the user would like to buy. The user will be prompted in the Steamworks Overlay to complete the purchase in their local currency, funding their Steamworks Wallet if necessary, etc.
        /// </para>
        /// <para>
        /// If the purchase process was started successfully, then m_ulOrderID and m_ulTransID will be valid in the SteamInventoryStartPurchaseResult_t callback.
        /// </para>
        /// </remarks>
        /// <param name="items">A collection of the items to be purchased</param>
        /// <param name="itemQuantity">A collection of the quantities of each item to be purchased ... length must match length of <paramref name="items"/> </param>
        /// <param name="callback">[Optional] returns the order ID and transaction ID if the purchase was started successfuly otherwise it contains the EResult code whcih may indicate the reason for failure.</param>
        public static void StartPurchase(IEnumerable<InventoryItemDefinitionCount> itemsToPurchase, Action<SteamInventoryStartPurchaseResult_t, bool> callback = null)
        {
            var items = new SteamItemDef_t[itemsToPurchase.Count()];
            var counts = new uint[items.Length];

            for (int i = 0; i < items.Length; i++)
            {
                var element = itemsToPurchase.ElementAt(i);
                items[i] = element.item.definitionID;
                counts[i] = element.count;
            }

            var call = SteamInventory.StartPurchase(items, counts, (uint)items.Count());
            if (callback == null)
            {
                m_SteamInventoryStartPurchaseResult.Set(call, (r,e) =>
                {
                    if(e)
                    {
                        Debug.LogError("An error occured while starting a purchase request: " + r.m_result);
                    }
                });
            }
            else
            {
                m_SteamInventoryStartPurchaseResult.Set(call, callback.Invoke);
            }
        }
        ...
        public static bool GetAllItems(Action<bool, SteamItemDetails_t[]> callback)
        {
            ...
            {
                Debug.LogError("Attempted call to Get All Items before the Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Get All Items from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        public static bool AddPromoItem(SteamItemDef_t itemDefinition, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Add Promo Item before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Add Promo Item from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        /// <para>Checks whether an inventory result handle belongs to the specified Steamworks ID.</para>
        /// </summary>
        /// <param name="resultHandle">The inventory result handle to check the Steamworks ID on.</param>
        /// <param name="steamIDExpected">The Steamworks ID to verify.</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="resultHandle">The inventory result handle to check the Steamworks ID on.</param>
        /// <param name="steamUserExpected">The Steamworks User Data to verify.</param>
        /// <returns></returns>
        public static bool CheckResultSteamID(SteamInventoryResult_t resultHandle, UserData steamUserExpected)
        {
        ...
        /// <param name="callback">The callback to invoke when the request has been completed by Steamworks Inventory Services</param>
        ...
        public static bool ConsumeItem(SteamItemInstanceID_t instanceId, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Consume Item before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            if (!result)
            {
                Debug.LogWarning("Failed to request Consume Item from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
            else
            {
        ...
        public static bool DeserializeResult(byte[] buffer, CSteamID fromUser, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Deserialize Result before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Deserialize Result from the Steamworks Steamworks Inventory service. This should never happen according to Steamworks documentaiton ... please contact Valve's partner support for more information.");
                return false;
            }
        }
        ...
        /// <param name="callback">The callback to call when completed</param>
        /// <returns>Always returns true</returns>
        public static bool DeserializeResult(byte[] buffer, UserData fromUser, Action<bool, SteamItemDetails_t[]> callback)
        {
        ...
        public static bool ExchangeItems(ItemExchangeRecipe recipe, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Exchange Items before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Exchange Items from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        /// <para>This API is only intended for prototyping - it is only usable by Steamworks accounts that belong to the publisher group for your game.</para>
        /// </summary>
        ...
        public static bool DeveloperOnlyGenerateItems(List<GenerateItemCount> ItemDefinitions, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Generate Items before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Generate Items from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        public static bool GetItemsByID(IEnumerable<SteamItemInstanceID_t> InstanceIDs, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Get Items By ID before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Get Items By ID from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        public static bool GrantPromoItems(Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Grant Promo Items before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Grant Promo Items from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        /// </summary>
        /// <param name="steamID">The Steamworks ID of the user to request the eligible promo items for.</param>
        ...
        public static bool TransferQuantity(SteamItemInstanceID_t sourceItem, uint quantityToMove, SteamItemInstanceID_t destinationItem, Action<bool> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Stack Items before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Stack Item (Transfer Item Quantity) from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
        ...
        public static bool TriggerItemDrop(SteamItemDef_t dropListDefinition, Action<bool, SteamItemDetails_t[]> callback)
        {
            if (!RegisterCallbacks())
            {
                Debug.LogError("Attempted call to Trigger Item Drop before the Steamworks Foundation Manager Initialized");
                return false;
            }
            ...
            {
                Debug.LogWarning("Failed to request Trigger Item Drop from the Steamworks Steamworks Inventory service. This call is not valid from a Steamworks Game Server");
                return false;
            }
        }
