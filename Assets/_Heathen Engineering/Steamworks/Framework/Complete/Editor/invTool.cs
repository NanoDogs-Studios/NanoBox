...
namespace HeathenEngineering.SteamAPI.Editors
{
    public class InventoryIMGUITool
    {
        ...
        private SteamworksInventorySettings settings { get => steamSettings != null ? steamSettings.client.inventory : null; }
        ...
        private void DrawItemEditor()
        {
            ...
            if (activeItem != null)
            {
                if (settings.itemDefinitions.Contains(activeItem))
                {
                    ...
                    var nID = EditorGUILayout.IntField(activeItem.definitionID.m_SteamItemDef);
                    if(nID != activeItem.definitionID.m_SteamItemDef)
                    {
                        Undo.RecordObject(activeItem, "id change");
                        activeItem.definitionID.m_SteamItemDef = nID;
        ...
        private void DrawSettingsArea()
        {
            ...
            if (GUILayout.Button("Generate JSON", EditorStyles.toolbarButton, GUILayout.Width(100)) && isValidModel)
            {
                ...
                targetPath = targetPath.Replace(steamSettings.name + ".asset", "InventoryItemDefinition.JSON");
